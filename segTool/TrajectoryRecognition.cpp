...
#include "TrajectoryRecognition.h"
using namespace std;
bool once;
...
int ReadDataFile(CString positionfile,int j,int n)//j:词语序号 n:人的序号
{
	...
	flhandx=glhandx[j][n][0];
	flhandy=glhandy[j][n][0];
	flhandz=glhandz[j][n][0];
	frhandx=grhandx[j][n][0];
	frhandy=grhandy[j][n][0];
	frhandz=grhandz[j][n][0];
...
void GetFileNames(CString firstP00,CString P00[WordsNum],HANDLE hFind,WIN32_FIND_DATA &FindFileData)
{
	//遍历P00的文件名
...
{
	//遍历获取文件名
	CString firstP00="D:\\iData\\Kinect sign data\\trajectory\\D3\\K50\\*.txt";
	CString firstP01="D:\\iData\\Kinect sign data\\trajectory\\D3\\K51\\*.txt";
	CString firstP02="D:\\iData\\Kinect sign data\\trajectory\\D3\\K52\\*.txt";
	CString firstP03="D:\\iData\\Kinect sign data\\trajectory\\D3\\K53\\*.txt";
	CString firstP04="D:\\iData\\Kinect sign data\\trajectory\\D3\\K54\\*.txt";
	//CString firstP00="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\2维\\P00\\*.txt";
	//CString firstP01="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\2维\\P01\\*.txt";
	//CString firstP02="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\2维\\P02\\*.txt";
	////CString firstP03="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\P03\\*.txt";
	//CString firstP03="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\2维\\P06\\*.txt";
	//CString firstP08="C:\\Users\\Lshun21\\Documents\\HCI\\手语识别\\数据\\候选词汇手工标注\\2维\\P08\\*.txt";
...
int CurveRecognision(float headx0, float heady0, float headz0, float lhandx0[],float lhandy0[], float lhandz0[],
	float rhandx0[],float rhandy0[], float rhandz0[],int n,double score[WordsNum]/*, int trajectoryMatchedIndex[][Posture_num][2]*/
	,int sentenceID,int startFrameID, int endFrameID)
{
	...
	//处理原始数据（转化为相对位置)
	RawdataProcess(headx0,heady0,headz0,lhandx0,lhandy0,lhandz0,rhandx0,rhandy0,rhandz0,n);
	const int loops=3;
	MedianFliter(lhandx[0],lhandy[0],lhandz[0],framenum[0]);
	MedianFliter(rhandx[0],rhandy[0],rhandz[0],framenum[0]);
	ReSample(lhandx[0],lhandy[0],lhandz[0],framenum[0],rsnum);
	...
	int galleryStartFrame[Gallery_num][WordsNum];
	int galleryEndFrame[Gallery_num][WordsNum];
	for (int j=0;j<WordsNum;j++)
	{
		//cout<<"word Index: "<<j<<endl;
		//clock_t starttime =clock(); 
		//ReadDataFile(P00[j],0);	
		if (j==55)
		{
			int p=0;
		}
		...
		for (int k=1;k<num;k++)
		{
			...
			d_seg = 1000000;
			framenum[k]=gframenum[j][k];
			galleryStartFrame[k][j] = -1;
			galleryEndFrame[k][j] = -1;   //Means none. It is an initial.
			double dl_seg = 10000.0;
			double dr_seg = 10000.0;
			int begin = KeyPostureBeginEnd[k][j][0][0];
			int end = KeyPostureBeginEnd[k][j][KeyPostureNum[k][j]-1][1];
			int gallerySize = (end - begin)>0?(end - begin):0;
			int probeSize =(endFrameID - startFrameID)>0?(endFrameID - startFrameID):0;
			CopyHandData_seg( begin,end,glhandx[j][k],glhandy[j][k],glhandz[j][k],grhandx[j][k],grhandy[j][k],grhandz[j][k],
			...
			double galleryLength = 0;
			double probeLength = 0;
			for (int i=1; i<rsnum; i++)
			{
				galleryLength += abs(lhandx[k][i] - lhandx[k][i-1]) 
					+ abs(lhandy[k][i] - lhandy[k][i-1])
					+ abs(lhandz[k][i] - lhandz[k][i-1])
					+ abs(rhandx[k][i] - rhandx[k][i-1])
					+ abs(rhandy[k][i] - rhandy[k][i-1])
					+ abs(rhandz[k][i] - rhandz[k][i-1]);

				probeLength += abs(lhandx[0][i] - lhandx[0][i-1]) 
					+ abs(lhandy[0][i] - lhandy[0][i-1])
					+ abs(lhandz[0][i] - lhandz[0][i-1])
					+ abs(rhandx[0][i] - rhandx[0][i-1])
					+ abs(rhandy[0][i] - rhandy[0][i-1])
					+ abs(rhandz[0][i] - rhandz[0][i-1]);
			}

			//if (gallerySize > allowedTraFrameSize)
			//if (abs(galleryLength-probeLength)<30)  //I think length is better than size.
			
			if (KeyPostureNum[k][j]!=0 && abs(galleryLength-probeLength)<10)
			{
				//int begin_,end_;
				//dl_seg=RandomPartialMatchDistance(k,lhandx[0],lhandy[0],lhandz[0],lhandx[k],lhandy[k],lhandz[k],begin_,end_);
				//dr_seg=RandomPartialMatchDistance(k,rhandx[0],rhandy[0],rhandz[0],rhandx[k],rhandy[k],rhandz[k],begin_,end_);
				//d_seg=DistanceOfSlidedCurves(k,lhandx[0],lhandy[0],lhandy[0],lhandx[k],lhandy[k],lhandz[k]
				//,rhandx[0],rhandy[0],rhandz[0],rhandx[k],rhandy[k],rhandz[k]);
				d_seg=DistanceOfGallerySlideProbe(k,lhandx[0],lhandy[0],lhandy[0],
					lhandx[k],lhandy[k],lhandz[k],
					rhandx[0],rhandy[0],rhandz[0],
 					rhandx[k],rhandy[k],rhandz[k]);
				galleryStartFrame[k][j] = 0;
				galleryEndFrame[k][j] = 0;
			}
			
			d_seg = sqrt(d_seg);
			//d_seg = sqrt(dl_seg+dr_seg);
			;
// 			for (int m=0; m<KeyPostureNum[k][j]; m++)
// 			{
// 				for (int n=m; n<KeyPostureNum[k][j]; n++)
// 				{
					//if (m == trajectoryMatchedIndex[k][j][0] && n==trajectoryMatchedIndex[k][j][1])
// 					{
// 						int begin = KeyPostureBeginEnd[k][j][m][0];
// 						int end = KeyPostureBeginEnd[k][j][n][1];
// 						CopyHandData_seg( begin,end,glhandx[j][k],glhandy[j][k],glhandz[j][k],grhandx[j][k],grhandy[j][k],grhandz[j][k],
// 							lhandx[k],lhandy[k],lhandz[k],rhandx[k],rhandy[k],rhandz[k]);
// 						int framenum_seg = end - begin;
// 						MedianFliter(lhandx[k],lhandy[k],lhandz[k],framenum_seg);
...
// 						double dl_seg = 0.0;
// 						double dr_seg = 0.0;
// 						dl_seg=DistanceOfTwoCurves(k,lhandx[0],lhandy[0],lhandz[0],lhandx[k],lhandy[k],lhandz[k],lE0,lE[j][k]);
// 						dr_seg=DistanceOfTwoCurves(k,rhandx[0],rhandy[0],rhandz[0],rhandx[k],rhandy[k],rhandz[k],rE0,rE[j][k]);
// 						d_seg = sqrt(dl_seg + dr_seg);
// 						if (d_seg < minD)
// 						{
// 							minD = d_seg;
// 							galleryStartFrame[k][j] = m;
// 							galleryEndFrame[k][j] = n;
// 						}
// 					}
//				}
//			}
		...
		//cout<<"read "<<static_cast<double>(endtime-starttime)/CLOCKS_PER_SEC*1000<<"ms"<<endl;
	}
	CString s_FileName;
	...
	//Folder to save key frame images.
	CString s_filefolder;
	s_filefolder.Format("..\\testKeyImage\\%d\\trajectory",sentenceID);
	_mkdir(s_filefolder);//Create the folder.
	s_filefolder.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d",sentenceID,startFrameID);
	_mkdir(s_filefolder);
	s_filefolder.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\allTheGallery1",sentenceID,startFrameID);
	_mkdir(s_filefolder);
	//-----------------------
	s_FileName.Format("..\\testKeyImage\\%d\\trajectory\\detailDistance.csv",sentenceID);
	...
	for (int j=0;j<WordsNum;j++)
	{
		...
		//if (galleryStartFrame[k][j] >0 && galleryEndFrame[k][j]>0)
		{
			...
			s_FileName_gallery.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\allTheGallery1\\gallery1_%d.txt",
	...
	s_FileName_probe.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\probe_%d_%d_%d.txt",
...
// 	outfile_probe.close();
	...
	s_FileName.Format("..\\testKeyImage\\%d\\trajectory\\rank5detail.txt",sentenceID);
...
// 	vector<float> probe_im_sort[6];
// 	vector<float> gallery_im_sort[6];
// 	for (int j=0; j<6; j++)
// 	{
// 		for (int k=0; k<rsnum; k++)
// 		{
// 			probe_im_sort[j].push_back(probe_im[j][k]);
// 			gallery_im_sort[j].push_back(gallery_im[j][k]);
// 		}
// 	}
// 	for (int j=0; j<6; j++)
// 	{
// 		sort(gallery_im_sort[j].begin(),gallery_im_sort[j].end());
// 		sort(probe_im_sort[j].begin(),probe_im_sort[j].end());
// 	}


// 	IplImage* galleryCurveXY = cvCreateImage(cvSize(640,480),8,3);
// 	cvSet(galleryCurveXY, CV_RGB(255,255,255), NULL);
// 	int scale = 200;
// 	for (int j=1; j<rsnum; j++)
// 	{
// 		float galleryXY_Left_X0 = (gallery_im[0][j-1]-gallery_im_sort[0][0])*scale+50;
// 		float galleryXY_Left_Y0 = (gallery_im[1][j-1]-gallery_im_sort[1][0])*scale+100;
// 		float probeXY_Left_X0 = (probe_im[0][j-1]-probe_im_sort[0][0])*scale+50;
// 		float probeXY_Left_Y0 = (probe_im[1][j-1]-probe_im_sort[1][0])*scale+100;
// 
// 		float galleryXY_Right_X0 = (gallery_im[3][j-1]-gallery_im_sort[3][0])*scale+320;
// 		float galleryXY_Right_Y0 = (gallery_im[4][j-1]-gallery_im_sort[4][0])*scale+100;
// 		float probeXY_Right_X0 = (probe_im[3][j-1]-probe_im_sort[3][0])*scale+320;
// 		float probeXY_Right_Y0 = (probe_im[4][j-1]-probe_im_sort[4][0])*scale+100;
// 
// 		float galleryXY_Left_X = (gallery_im[0][j]-gallery_im_sort[0][0])*scale+50;
// 		float galleryXY_Left_Y = (gallery_im[1][j]-gallery_im_sort[1][0])*scale+100;
// 		float probeXY_Left_X = (probe_im[0][j]-probe_im_sort[0][0])*scale+50;
// 		float probeXY_Left_Y = (probe_im[1][j]-probe_im_sort[1][0])*scale+100;
// 
// 		float galleryXY_Right_X = (gallery_im[3][j]-gallery_im_sort[3][0])*scale+320;
// 		float galleryXY_Right_Y = (gallery_im[4][j]-gallery_im_sort[4][0])*scale+100;
// 		float probeXY_Right_X = (probe_im[3][j]-probe_im_sort[3][0])*scale+320;
// 		float probeXY_Right_Y = (probe_im[4][j]-probe_im_sort[4][0])*scale+100;
// 
// 		cvLine(galleryCurveXY,cvPoint(galleryXY_Left_X0,galleryXY_Left_Y0),cvPoint(galleryXY_Left_X,galleryXY_Left_Y),CV_RGB(0,0,0),1,0);
// 		cvLine(galleryCurveXY,cvPoint(probeXY_Left_X0,probeXY_Left_Y0),cvPoint(probeXY_Left_X,probeXY_Left_Y),CV_RGB(255,0,0),1,0);
// 
// 		cvLine(galleryCurveXY,cvPoint(galleryXY_Right_X0,galleryXY_Right_Y0),cvPoint(galleryXY_Right_X,galleryXY_Right_Y),CV_RGB(0,255,0),1,0);
// 		cvLine(galleryCurveXY,cvPoint(probeXY_Right_X0,probeXY_Right_Y0),cvPoint(probeXY_Right_X,probeXY_Right_Y),CV_RGB(255,0,0),1,0);
// 		
// 	}
// 	s_ImgFileName.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\XY_%d_%d_%d.jpg",sentenceID,startFrameID,startFrameID,endFrameID,maxClass);
// 	cvSaveImage(s_ImgFileName,galleryCurveXY);
// 	cvReleaseImage(&galleryCurveXY);
// 
// 	//////////////////////////////////////////////////////////////////////////
// 	IplImage* galleryCurveYZ = cvCreateImage(cvSize(640,480),8,3);
// 	cvSet(galleryCurveYZ, CV_RGB(255,255,255), NULL);
// 	//int scale = 200;
// 	for (int j=1; j<rsnum; j++)
// 	{
// 		float galleryYZ_Left_Y0 = (gallery_im[1][j-1]-gallery_im_sort[1][0])*scale+50;
// 		float galleryYZ_Left_Z0 = (gallery_im[2][j-1]-gallery_im_sort[2][0])*scale+100;
// 		float probeYZ_Left_Y0 = (probe_im[1][j-1]-probe_im_sort[1][0])*scale+50;
// 		float probeYZ_Left_Z0 = (probe_im[2][j-1]-probe_im_sort[2][0])*scale+100;
// 
// 		float galleryYZ_Right_Y0 = (gallery_im[4][j-1]-gallery_im_sort[4][0])*scale+320;
// 		float galleryYZ_Right_Z0 = (gallery_im[5][j-1]-gallery_im_sort[5][0])*scale+100;
// 		float probeYZ_Right_Y0 = (probe_im[4][j-1]-probe_im_sort[4][0])*scale+320;
// 		float probeYZ_Right_Z0 = (probe_im[5][j-1]-probe_im_sort[5][0])*scale+100;
// 
// 		float galleryYZ_Left_Y = (gallery_im[1][j]-gallery_im_sort[1][0])*scale+50;
// 		float galleryYZ_Left_Z = (gallery_im[2][j]-gallery_im_sort[2][0])*scale+100;
// 		float probeYZ_Left_Y = (probe_im[1][j]-probe_im_sort[1][0])*scale+50;
// 		float probeYZ_Left_Z = (probe_im[2][j]-probe_im_sort[2][0])*scale+100;
// 
// 		float galleryYZ_Right_Y = (gallery_im[4][j]-gallery_im_sort[4][0])*scale+320;
// 		float galleryYZ_Right_Z = (gallery_im[5][j]-gallery_im_sort[5][0])*scale+100;
// 		float probeYZ_Right_Y = (probe_im[4][j]-probe_im_sort[4][0])*scale+320;
// 		float probeYZ_Right_Z = (probe_im[5][j]-probe_im_sort[5][0])*scale+100;
// 
// 		cvLine(galleryCurveYZ,cvPoint(galleryYZ_Left_Y0,galleryYZ_Left_Z0),cvPoint(galleryYZ_Left_Y,galleryYZ_Left_Z),CV_RGB(0,0,0),1,0);
// 		cvLine(galleryCurveYZ,cvPoint(probeYZ_Left_Y0,probeYZ_Left_Z0),cvPoint(probeYZ_Left_Y,probeYZ_Left_Z),CV_RGB(255,0,0),1,0);
// 
// 		cvLine(galleryCurveYZ,cvPoint(galleryYZ_Right_Y0,galleryYZ_Right_Z0),cvPoint(galleryYZ_Right_Y,galleryYZ_Right_Z),CV_RGB(0,255,0),1,0);
// 		cvLine(galleryCurveYZ,cvPoint(probeYZ_Right_Y0,probeYZ_Right_Z0),cvPoint(probeYZ_Right_Y,probeYZ_Right_Z),CV_RGB(255,0,0),1,0);
// 
// 	}
// 	s_ImgFileName.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\YZ_%d_%d_%d.jpg",sentenceID,startFrameID,startFrameID,endFrameID,maxClass);
// 	cvSaveImage(s_ImgFileName,galleryCurveYZ);
// 	cvReleaseImage(&galleryCurveYZ);
// 	//////////////////////////////////////////////////////////////////////////
// 	IplImage* galleryCurveXZ = cvCreateImage(cvSize(640,480),8,3);
// 	cvSet(galleryCurveXZ, CV_RGB(255,255,255), NULL);
// 	//int scale = 200;
// 	for (int j=1; j<rsnum; j++)
// 	{
// 		float galleryXZ_Left_X0 = (gallery_im[0][j-1]-gallery_im_sort[0][0])*scale+50;
// 		float galleryXZ_Left_Z0 = (gallery_im[2][j-1]-gallery_im_sort[2][0])*scale+100;
// 		float probeXZ_Left_X0 = (probe_im[0][j-1]-probe_im_sort[0][0])*scale+50;
// 		float probeXZ_Left_Z0 = (probe_im[2][j-1]-probe_im_sort[2][0])*scale+100;
// 
// 		float galleryXZ_Right_X0 = (gallery_im[3][j-1]-gallery_im_sort[3][0])*scale+320;
// 		float galleryXZ_Right_Z0 = (gallery_im[5][j-1]-gallery_im_sort[5][0])*scale+100;
// 		float probeXZ_Right_X0 = (probe_im[3][j-1]-probe_im_sort[3][0])*scale+320;
// 		float probeXZ_Right_Z0 = (probe_im[5][j-1]-probe_im_sort[5][0])*scale+100;
// 
// 		float galleryXZ_Left_X = (gallery_im[0][j]-gallery_im_sort[0][0])*scale+50;
// 		float galleryXZ_Left_Z = (gallery_im[2][j]-gallery_im_sort[2][0])*scale+100;
// 		float probeXZ_Left_X = (probe_im[0][j]-probe_im_sort[0][0])*scale+50;
// 		float probeXZ_Left_Z = (probe_im[2][j]-probe_im_sort[2][0])*scale+100;
// 
// 		float galleryXZ_Right_X = (gallery_im[3][j]-gallery_im_sort[3][0])*scale+320;
// 		float galleryXZ_Right_Z = (gallery_im[5][j]-gallery_im_sort[5][0])*scale+100;
// 		float probeXZ_Right_X = (probe_im[3][j]-probe_im_sort[3][0])*scale+320;
// 		float probeXZ_Right_Z = (probe_im[5][j]-probe_im_sort[5][0])*scale+100;
// 
// 		cvLine(galleryCurveXZ,cvPoint(galleryXZ_Left_X0,galleryXZ_Left_Z0),cvPoint(galleryXZ_Left_X,galleryXZ_Left_Z),CV_RGB(0,0,0),1,0);
// 		cvLine(galleryCurveXZ,cvPoint(probeXZ_Left_X0,probeXZ_Left_Z0),cvPoint(probeXZ_Left_X,probeXZ_Left_Z),CV_RGB(255,0,0),1,0);
// 
// 		cvLine(galleryCurveXZ,cvPoint(galleryXZ_Right_X0,galleryXZ_Right_Z0),cvPoint(galleryXZ_Right_X,galleryXZ_Right_Z),CV_RGB(0,255,0),1,0);
// 		cvLine(galleryCurveXZ,cvPoint(probeXZ_Right_X0,probeXZ_Right_Z0),cvPoint(probeXZ_Right_X,probeXZ_Right_Z),CV_RGB(255,0,0),1,0);
// 
// 	}
// 	s_ImgFileName.Format("..\\testKeyImage\\%d\\trajectory\\startFrame_%d\\XZ_%d_%d_%d.jpg",sentenceID,startFrameID,startFrameID,endFrameID,maxClass);
// 	cvSaveImage(s_ImgFileName,galleryCurveXZ);
// 	cvReleaseImage(&galleryCurveXZ);
...
void CurveRecognition(CvPoint3D32f headPoint3D, vector<SLR_ST_Skeleton> vSkeletonData, 
	double score[WordsNum], /*int trajectoryMatchedIndex[][Posture_num][2],*/int sentenceID,int startFrameID, int endFrameID)
{
	int length = vSkeletonData.size();
	float *lhandx0 = new float[length];
	float *lhandy0 = new float[length];
	...
	CurveRecognision(headx0,heady0,headz0,lhandx0,lhandy0,lhandz0,rhandx0,rhandy0,rhandz0,length,score/*, trajectoryMatchedIndex*/
...
//------------------------
void Get_Probe_IM(float probe_im[6][rsnum])
{
	CopyHandData(plhandx,plhandy,plhandz,prhandx,prhandy,prhandz,lhandx[0],lhandy[0],lhandz[0],rhandx[0],rhandy[0],rhandz[0]);
	const int loops=3;
	MedianFliter(lhandx[0],lhandy[0],lhandz[0],framenum[0]);
	...
	float rxk[],float ryk[],float rzk[])
{
	...
	double delta=0.2;//probe波动的幅值
	double m=0.1;//probe波动的步幅
	double xita=0.9;//rate_flu>xita时，不比较,认为不相似。
