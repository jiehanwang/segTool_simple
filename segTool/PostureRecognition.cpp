...
IplImage* Extract_image(int index1,int index2)//从关键帧序列中返回代表图像
{
	...
	IplImage * avg_img=cvCreateImage( cvSize(iSIZE,iSIZE),8,1);//均值图像
	uchar *pp;
	uchar *qq;
	int Img_sum[iSIZE][iSIZE];//用于图像求和
...
bool GetHOGHistogram_Patch(IplImage *img,double hog_hist[])//取得图像img的HOG特征向量
{
	...
	HOGDescriptor *hog=new HOGDescriptor(cvSize(iSIZE,iSIZE),cvSize(16,16),cvSize(8,8),cvSize(8,8),9);
